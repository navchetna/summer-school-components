...
from model_zoo.model_api.models.utils import resize_image
...
class FaceDetector(Module):
    ...
    def preprocess(self, frame):
        ...
        return landmark_detector.resize_input(frame, self.input_shape, self.nchw_layout)
...
class LandmarksDetector(Module):
    ...
    def preprocess(self, frame, rois):
        inputs = landmark_detector.cut_rois(frame, rois)
        inputs = [landmark_detector.resize_input(input, self.input_shape, self.nchw_layout) for input in inputs]
        return inputs
...
class FrameProcessor:
    ...
    def __init__(self,m_fd,fd_input_size,t_fd,exp_r_fd,d_fd,allow_grow,no_showd,d_lm,m_lm):
...
class landmark_detector:
    ...
    def cut_rois(frame, rois):
        return [landmark_detector.crop(frame, roi) for roi in rois]
    ...
    def draw_detections(frame, frame_processor, detections, output_transform):
        ...
        for roi, landmarks in zip(*detections):
            ...
            for point in landmarks:
                ...
                x_i=int(x)
                y_i=int(y)
                landmarks_coordinates.append('{:4},{:4}'.format(x_i,y_i))
                print(landmarks_coordinates)
            cv2.rectangle(frame, (xmin, ymin), (xmax, ymax),(0,0,255), 2)
            
            face_coordinates.append("{"+"'name':"+","+"'face'"+","+"'face coordinates':"+"["+'{:4},{:4},{:4},{:4} '.format(xmin, ymin, xmax, ymax)+"]"+","+"'landmark coordinates':"+str(landmarks_coordinates)+"}")
            #print(coordinates)
        return frame,face_coordinates
    ...
    def landmark_detect(self,model_path_face_detect: str,model_path_landmark: str,VEDIO_PATH: str,DEVICE_FACE_DETECT: str,DEVICE_LANDMARK: str):
        try:
            self.m_fd=model_path_face_detect
            self.m_lm=model_path_landmark

            self.IN=VEDIO_PATH
            self.d_fd=DEVICE_FACE_DETECT
            self.d_lm=DEVICE_LANDMARK

            cap = open_images_capture(self.IN,self.loop)
            frame_processor = FrameProcessor(self.m_fd,self.fd_input_size,self.t_fd,self.exp_r_fd,self.d_fd,self.allow_grow,self.no_show,self.d_lm,self.m_lm)
            ...
            while True:
                ...
                frame,coordinates= landmark_detector.draw_detections(frame, frame_processor, detections, output_transform)
