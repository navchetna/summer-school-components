...
embeddings_list = embedding_model.encode(documents)




my_db = VectorDB(path = 'vector_database/data', collection_name = "test2", embedding_size=embeddings_list[0].shape[0])
...
top_k = my_db.query_embeddings(query_embedding)

top_chunks_list = [point.payload['text'] for point in top_k]

print(top_chunks_list)